...
NoShuffleArray(array) {
  ...
  handleAnswerSelected(answerContent,questionId,event) {
    ...
    // Answer content could be either an object or a string depending on the quiz type 
    if (typeof answerContent === 'object'){
        var test = Object.keys(answerContent) // gives an array 
    }
    else {
      var test = Array(event.currentTarget.value)
    } 

    if (this.state.quizQuestionsBlock[this.state.counter].condition===true) { // Next question is defined based on the previous answer 
      
      // Loop through an array and compare values to the condition_value 
      const resultat = test.map(x => this.state.quizQuestionsBlock[this.state.counter].condition_value.includes(x));
      
      // var resultat = this.state.quizQuestionsBlock[this.state.counter].condition_value.includes(test);
      const resultatt = resultat.map(x => String(x));
 
      
    if (resultatt.includes("true")){  // answer requires the next question 
        step = 1
        
     }
     else {
      step = 2 // Skip the next question 
  ...
  setNextQuestion() {
    ...
    this.setState({
      ...
      image: image_item,
      image_a: image_a,
      step: 1 // put step to 1 by default 
    });
  }
